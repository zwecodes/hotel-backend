...
router.get('/profile', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: { ...users[0], stats: stats[0] },
    });
  } catch (error) {
    logger.error('Profile Error', { error: error.message, userId: req.user.id });
...
router.put('/profile/avatar', authMiddleware, async (req, res) => {
  try {
    ...
    logger.error('Update Avatar Error', { error: error.message, userId: req.user.id });
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
